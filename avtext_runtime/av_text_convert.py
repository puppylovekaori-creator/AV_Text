...
def normalize_and_tokenize(
    ...
    *,
    ...
    # 既知名の分割補助
    if for_registration:
        # registration_text は「セパレータ無し連結時」のみ強め分割。
        # 既に区切りがある状態で強め分割をかけると、内包名で割れる事故を起こす。
        if has_people_separators(raw):
...
def main() -> None:
    ...
    # --- エイリアス読み込み（DB優先） ---
    try:
        alias_repo = AliasRepository(logger)
        aliases = alias_repo.load(conn, aliases_csv)
    except Exception as e:
